...
import { TbFileText, TbLayoutDashboard } from "react-icons/tb";

// 1. Definisikan Props yang diterima dari App.tsx
...
const Navbar = ({ isCompact, toggleView }: NavbarProps) => {
    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex-shrink-0 cursor-pointer">
                        ...
                        >
                            {/* Kalau ada logo image, pakai di sini. Kalau teks: */}
                            ALIF
                            <span className="w-2 h-2 rounded-full bg-[#FD6F00] mt-1"></span>
                        </a>
                    </div>

                    {/* DESKTOP MENU (Hanya muncul jika TIDAK dalam mode Compact) */}
                    {!isCompact && (
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        {/* --- BARU: Toggle Resume/Creative Mode --- */}
                        <button
                            onClick={toggleView}
                            className="flex items-center gap-2 px-3 py-2 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-[#FD6F00] hover:text-white dark:hover:text-white transition-all duration-300 text-xs md:text-sm font-medium border border-gray-200 dark:border-gray-700"
                        >
                            {isCompact ? <TbLayoutDashboard size={18} /> : <TbFileText size={18} />}
                            <span className="hidden sm:inline">
                                {isCompact ? "Creative Mode" : "Resume Mode"}
                            </span>
                        </button>
                        ...
                        {/* Hamburger Button (Hanya muncul jika TIDAK dalam mode Compact) */}
                        {!isCompact && (
                            <motion.button
                                whileTap={{ scale: 0.9 }}
            ...
            {/* MOBILE MENU (Hanya muncul jika TIDAK dalam mode Compact) */}
            <AnimatePresence>
                {isOpen && !isCompact && (
                    <motion.div
